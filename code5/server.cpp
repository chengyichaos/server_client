...
void server::HandleRequest(int conn,string str){ //conn与客户端通信的套接子
    char buffer[1000];
    string name,pass;
    ...
    if(!con){
        cout<<"mysql_init失败"<<endl;
    }
    if(!(mysql_real_connect(con,"127.0.0.1","root","931110","ChatProject",0,NULL,CLIENT_MULTI_STATEMENTS))){
        cout<<"连接失败"<<endl;
